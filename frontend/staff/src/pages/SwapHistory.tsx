                ...
                paginatedOrders.map((order) => (
                  <tr key={order.id} className="bg-white hover:bg-gray-50/50 dark:bg-gray-800 dark:hover:bg-gray-700/30 transition-colors">
                    ...
                    {visibleColumns.oldBattery && (
                      <td className="px-6 py-4 font-mono font-medium text-gray-600 dark:text-gray-300">{order.oldBatterySerial}</td>
                    )}
                    {visibleColumns.newBattery && (
                      <td className="px-6 py-4 font-mono font-bold text-blue-600 dark:text-blue-400">{order.newBatterySerial}</td>
                    )}
                    {visibleColumns.createdAt && (
                      <td className="px-6 py-4 text-xs text-gray-500">{new Date(order.createdAt).toLocaleString('vi-VN')}</td>
                    )}
                    {visibleColumns.status && (
                      <td className="px-6 py-4"><span className="px-2.5 py-1 text-xs font-bold rounded-full bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300">{order.status}</span></td>
                    )}
                  </tr>
                ))
              )}
        ...
        {!loading && totalElements > 0 && (
          <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-700/50 flex flex-col sm:flex-row justify-between items-center gap-4 bg-gray-50/20 dark:bg-gray-900/10">
            <div className="flex items-center gap-3">
              <span className="text-sm text-gray-500 dark:text-gray-400">Trang <span className="font-semibold text-gray-900 dark:text-white">{currentPage + 1}</span> / {totalPages || 1}</span>
              <select
                value={pageSize}
            ...
            {totalPages > 1 && (
              <div className="flex gap-2">
                <button onClick={() => setCurrentPage((p) => Math.max(0, p - 1))} disabled={currentPage === 0} className="px-4 py-2 text-sm font-semibold bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-sm active:scale-95">← Trước</button>
                <button onClick={() => setCurrentPage((p) => Math.min(totalPages - 1, p + 1))} disabled={currentPage >= totalPages - 1} className="px-4 py-2 text-sm font-semibold bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-sm active:scale-95">Tiếp →</button>
              </div>
            )}
          </div>
        )}
